...
- **Configuration.** The scaffold ships with `liaison-mode: cto`, `engineering: ralph`, and `observer: none` in `.claude/sdg-config.md`. Offer the alternatives — `pm` (Developer keeps the major technical decisions), `workflow` (a dynamic-workflows-native build for the engineering phases; requires the Workflow tool in the environment that will run the process), and `observer: tracker` (progress telemetry to a dashboard endpoint via `SDG_OBSERVER_URL`) — and update the config if the user wants them; otherwise leave the defaults.
