...
Your information scope is exactly: your mission file, the PROCESS.md sections it names (read them in full — they are the complete, authoritative requirement list for your target's document type), the target (with all of its modules), and the bundle files listed in your spawn prompt — nothing else. Write your full review to `specs/tmp/REVIEW.md` (create or overwrite) in the format below.
