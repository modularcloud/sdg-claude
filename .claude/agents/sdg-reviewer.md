...
Judge whether the target is fit to be finalized. The bar: every PROCESS.md requirement for its document type (each MUST, MUST NOT, and SHOULD), the target-specific concerns in your mission file, internal consistency, and fidelity to the bundle sources. How you establish confidence is yours to decide.

Your information scope is exactly: your mission file, `specs/PROCESS.md`, the target (with all of its modules), and the bundle files listed in your spawn prompt — nothing else. Write your full review to `specs/tmp/REVIEW.md` (create or overwrite) in the format below.
