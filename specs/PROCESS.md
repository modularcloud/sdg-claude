*This process is running in Claude. For Claude-Specific instructions, see @.claude/prompts/PROCESS.md. This document is a conceptual guide and may differ slightly from how it is run in Claude so both documents should be read together holistically.*
...
- It MUST NOT reference or assume knowledge of prior versions of the itself.
- It MUST NOT contain migration details from previous version of itself, unless migration is an explicitly supported feature.
...
- It MUST specify the product in a way that is fully blackbox testable, exposing testing seams that enable all behaviors to be tested through E2E tests. In other words, the interface and contracts must be sufficiently powerful to expose routes to test all requirements and edge cases though blackbox means. This includes adding seams and options that are intended for a tester and not necessarily the target customer.
- Test seems MUST be specified in a manner that does not introduce security vulnerabilities.
...
- They MAY be nested. In other words, a module (the parent) can be further modularized into (child) sub-modules. The parent MUST link to the children. Regardless of the nested relationship, all modules MUST live in the flat `module/` directory.
...
## `TEST-SPEC.md`
`TEST-SPEC.md` is the source of truth for test harness.
...
- It MUST NOT add or remove product requirements defined `SPEC.md`.
...
- It MUST include internal self-tests to increase confidence that the test harness is properly implemented prior to product implementation.
...
- It MUST NOT skip tests that cannot run in GitHub CI, instead those test MUST be implemented as local-only tests.
...
For example, if someone wants to add a "goblin mode" feature to the product, and there are already 11 existing patches, then the patch could be titled `0012-goblin-mode.md`. Short titles must be unique and the index is padded with 4 zeros and is the `previous index + 1`, starting at index 1.
...
- They MAY suggest changes to `TEST-SPEC.md` (including modules, if relevant), but these suggestion MUST be made with the same rigor and methodology that `TEST-SPEC.md` is required to maintain.
- They MAY specify that no changes to `TEST-SPEC.md` is necessary. For example, a test could be correctly specified but the implementation of that test could be wrong.
# Actors
## Developer
...
## Specialist
Specialist is a general agent that performs one-off tasks in a manner that does not bloat the context of thread that spawns it.
# Sub-Processes
...
`SPEC.md`, `TEST-SPEC.md`, IPs, and bug reports and improved by iterative refinement prior to being finalized. This occurs when they are first created and when they are updated.
...
Driver should generally not look at implemented code, however, if iteratively refining a bug then it can do to confirm the bug report is on the right track. However, that doesn't mean that the bug report should contain specific fix prescriptions.
### Reviewer Prompts
...
*Note: `SPEC.md` includes all spec modules and `TEST-SPEC.md` includes all test modules.*
...
**Notes:**
1. "finishes the iteration" means that it commits any changes it made, pushes to the remote branch, performs no further work, and the ends the current iteration. This doesn't mean the ralph loop is done, it means that a new iteration will take over where it left off.
2. Determining if the implementation is in full spec compliance requires using multiple subagents to comprehensively review the relevant spec and implemented code to subjectively determine if the code fully meets all requirements. Aside from the agent reviewing the implementation and spec, the implementation must pass all tests required tests locally and in CI. When implementing the test harness, this means all test harness self-tests must pass. When implementing the product, this means **all** tests must pass.
...
**Iteration Flow:**
1. Reviews `SPEC.md`, `TEST-SPEC.md`, and `IMPLEMENTATION.md` and the goal it is given.
2. Reviews `FIX_PLAN.md`, if exists.
3. If `FIX_PLAN.md` does not exists, it determines if the implementation is in full spec compliance. If it is in full compliance, it runs the Code Review Sub-Flow. If the spec is not compliant, it creates a granular plan based on the findings to implement the spec and saves it to `FIX_PLAN.md`. To keep the context window short, it then finishes the iteration.
...
3. Adds all remaining ones to `FIX_PLAN.md` and finishes the iteration (does not implement changes in this iteration).
4. Commits and exits ralph loop. To be clear, at this point there should be no non-rejected code review comments, no `FIX_PLAN.md`, and there should be full spec compliance.

# Process
*When this process is run as part of an update/patch, some the stage must be tracked in the patch document. **Start Stage:**... indicates that at the beginning of the phase, the patch document tracking must be set to the specified stage. **End Stage:**... indicates that at the end of the phase, the patch document tracking must be set to the specified stage*
## Phase 0
...
3. If `SPEC.md` exists, and the seed describes a change that does not require an update to `SPEC.md`, this is classified as an **bug** (patch)

If it is the initial build, Specialist drafts the initial `SPEC.md` and commits when done. Then, jump to phase 4.

If it is patch, Specialist drafts the initial patch document for the improvement proposal or bug and commits it to a new branch with a name in the format `patch/short-title` and uses the Asking Developer process to resolve ambiguities. This patch document will be referred to as "the patch" for the rest of this workflow.
...
If, at the end of the iterative refinement process, the patch is classified as a bug that requires changes to `TEST-SPEC.md`, jump to phase 6. If no `TEST-SPEC.md` changes are needed, list the fixes that need to be made to the test harness in `FIX_PLAN.md` and jump to phase 8.
...
If Driver finds a blocking issues due to a problem in the patch, log the problem to `PATCH-PROBLEMS.md` and jump back to phase 3.
## Phase 5
...
If Driver finds a blocking issue due to a problem in `SPEC.md`, log the problem to `SPEC-PROBLEMS.md` and jump back to the phase 4.
## Phase 7
Specialist updates scaffolding based on the requirements in `SPEC.md`, `TEST-SPEC.md`, and `IMPLEMENTATION.md`, if needed.
...
## Phase 8
**Start Stage (Bug without `TEST-SPEC.md` changes):** Accepted
**Start Stage (Other):** Tests Specified
**End Stage:** Tested

Use a ralph loop to ensure the test harness implementation is fully adherent to `TEST-SPEC.md`, but do not work on the product implementation. All harness self-tests must pass and product tests may fail before completion.
...
If Engineer finds a blocking problem in `SPEC.md`, log it to `SPEC-PROBLEMS.md` and jump back phase 4. 
