...
* **T7-4 Globs.** Semantics fixtures: `*` within one segment (matches empty run); `?` exactly one character; `**` whole segments including none; case-sensitive matching; a dotfile matched only by a pattern segment written with a leading `.` — wildcards never match dot-segments: `a/**/b.mdx` does not match `a/.h/b.mdx`, `*` does not match `.hidden`, `?x` does not match `.x`; a pattern resolving outside the workspace root → 14.14. All paths resolve relative to the configuration file's directory.
