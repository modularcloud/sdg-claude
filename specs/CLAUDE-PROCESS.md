...
15. An optional observer (default `none`) streams run telemetry: Orchestrator-constructed status events plus fork-composed summary events. Strictly fail-open and side-effect-free toward the process; with `observer: none`, nothing loads and nothing emits.
