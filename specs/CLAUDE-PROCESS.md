...
| `OUTCOME: TASK_REMOVED — <task>` | Engineer | Task was already implemented (or invalid) and was removed |
| `OUTCOME: QUESTION` | Driver, Specialist, Liaison-bound agents (never Engineer, never Reviewer) | Paused on a Developer-intent question; body contains a `QUESTION FOR DEVELOPER` block |
...
| `OUTCOME: ERROR — <what>` | any agent | Malformed spawn (e.g., no mission file named); the Orchestrator corrects the spawn prompt and re-spawns fresh |
...
3. Liaison attempts to answer on Developer's behalf from chat history, `specs/PHILOSOPHY.md`, and the relevant documents.
   - Confident → returns `OUTCOME: ANSWER — <answer>`.
...
- **An ASK is pending** → relay the message verbatim to that Liaison episode via SendMessage; continue the pipeline above. Liaison decides whether the message answers the open question or is an interruption: open questions are never dropped — if the message is something else, Liaison handles it and re-surfaces the question in the same turn, reading genuinely ambiguous messages as answers first.
...
loop:
  if specs/tmp/FIX_PLAN.md does not exist:
      A. Compliance determination — spawn in parallel:
         - one sdg-specialist (mission specialist/compliance-review.md) per spec/test
           module in scope, plus one for the core document(s); each returns COMPLIANT
           or a list of gaps with citations
...
2. Accepted comments remain → sdg-engineer (mission `engineer/plan.md`) appends them to `specs/tmp/FIX_PLAN.md`; the ralph loop continues (they are implemented in later iterations, not now).
...
- **Phase 0 — Audit.** Spawn sdg-specialist (mission `specialist/audit.md`): pull the active branch, inventory the workspace, and classify it into **exactly one state** — first matching row wins; any others are noted as anomalies:
  ...
  |---|---|---|
  | Not bootstrapped | `specs/PROCESS.md`, this file, or any of the `.claude/agents/` / `.claude/prompts/` files missing | Report to Developer that the scaffold is incomplete; halt |
  | Refinement reverted | Any `specs/tmp/*-PROBLEMS.md` exists | The phase for the most upstream problems file (jump table above) |
  | Active patch | A patch in `specs/patches/` has a `Stage:` other than Complete | By stage — Proposed → 3; Accepted → IP: 4, Bug: 6, 7, or 9 per the needs stated in the patch; Applied → 6; Tests Specified → 9 (through 8 if scaffolding is missing); Tested → 10; Implemented → 11 |
  | Ready for triage | `specs/tmp/SEED.md` exists | Phase 2 |
  | Initial build in flight | No seed, no active patch, initial build incomplete | No `SPEC.md` → Phase 1; `SPEC.md` but no `TEST-SPEC.md` → 5; `TEST-SPEC.md` but harness incomplete → 9 (through 8 if needed); harness green but product not → 10 |
  | Complete / idle | Specs exist; no seed, no active patch, no temp files | Idle — route the Developer's message per §5 |

  Audit rules: if several patches are non-Complete, the most recently modified one is the active patch — flag the rest. Verify the checked-out branch matches the active patch (`patch/<short-title>`); flag mismatches rather than fixing them. Flag unpaired modules (a `MODULE.md` without its `TEST-MODULE.md`, or vice versa), uncommitted changes, and unexpected `specs/tmp/` files. Delete a stale `specs/tmp/REVIEW.md` — a fresh review is generated when refinement resumes. An interrupted iteration needs no special handling: the resumed phase re-verifies from files. Return `RESUME AT: phase <n>[, patch <file>] — <reason>` plus a short state summary listing every anomaly.
...
- **Phase 7 — Certifications.** If new/updated entries may be needed: sdg-specialist (mission `specialist/certification-draft.md`). Then §6, target CERTIFICATIONS.md. Problems → jump table. On HALT: `Stage: Tests Specified`.
...
- **Commits:** `sdg(phase-N): <imperative summary>`. Agents commit only files their mission touched and always push before finishing.
